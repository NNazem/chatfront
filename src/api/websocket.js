...
export async function findAndDisplayConversations(nickName) {
  const response = await apiRequest(
    `/searchConversation?nickName=${nickName}`,
    "GET"
  );
  return response;
}
...
export async function findLastMessage(senderId, recipientId) {
  console.log(senderId, recipientId);
  const response = await apiRequest(
    `/messages/lastmessage/${senderId}/${recipientId}`,
    "GET"
  );
...
export function connectWebSocket(
  ...
  stompClient.connect(headers, () => onConnected(stompClient, user), onError);
}
